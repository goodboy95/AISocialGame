...
python3 "$repo/ci/verify-production-runtime-contract.py" "$out/release/production-runtime-contract.json" "$out/docker-compose.yml" ai-social-game "$out/backend/start-production-backend.sh"
python3 - "$out" <<'PY'
