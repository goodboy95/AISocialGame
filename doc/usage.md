...
> Compose 会同时启动一个 `gateway`（nginx）容器监听宿主机 80 端口，负责将 `/` 请求代理到前端、`/api/*` 与 `/ws/*` 代理到后端。如需复现旧文档中的 `http://socialgame.seekerhut.com` 域名，可在本地 `hosts` 文件写入 `127.0.0.1 socialgame.seekerhut.com` 即可沿用同一配置。
